        ...
        for h, vv in self.get_all_eqs().items():
            if h == ():  # pylint: disable=g-explicit-bool-comparison
                for v1, v2 in vv:
                    ...
                    why_dict = minus(
                        {v1: 1, v2: -1}, minus(self.v2e[v1], self.v2e[v2]))
...
class AngleTable(GeometricTable):
    ...
    def add_eqangle(
        ...
        l1, l2, l3, l4 = [d._obj.num for d in [
            d1, d2, d3, d4]]  # pylint: disable=protected-access
    ...
    def get_all_eqs_and_why(self) -> Generator[Any, None, None]:
        ...
        for (_, vves1), (_, vves2) in perm2(list(h2pairs.items())):
            ...
            why_dict = minus(
                ...
                # there is no modulo, so this is 0
                minus(mult(e12, d), mult(e34, n)),
            )
