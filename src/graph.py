...
class Graph:
    ...
    def copy(self) -> Graph:
        ...
        g, _ = Graph.build_problem(
            p, definitions, verbose=False, init_copy=False)
    ...
    def get_or_create_const_rat(self, n: int, d: int) -> None:
        ...
        if (d, n) not in self.rconst:
            self._create_const_rat(
                d, n)  # pylint: disable=arguments-out-of-order
    ...
    def add_eqrat_const(
        ...
        for x, y, xy in [(a, b, ab), (c, d, cd)]:
            ...
            if {x, y} == {x_, y_}:
                continue
            if deps:
                deps = deps.extend(self, 'rconst', list(
                    args), 'cong', [x, y, x_, y_])
            args[2 * i - 2] = x_
            args[2 * i - 1] = y_
    ...
    def derive_algebra(
        ...
        eqs = {'eqangle': derives.pop(
            'eqangle'), 'eqratio': derives.pop('eqratio')}
        return derives, eqs
    ...
    def add_perp(
        ...
        for x, y, xy in [(a, b, ab), (c, d, cd)]:
            ...
            if {x, y} == {x_, y_}:
                continue
            if deps:
                deps = deps.extend(self, 'perp', list(args),
                                   'para', [x, y, x_, y_])
            args[2 * i - 2] = x_
            args[2 * i - 1] = y_
    ...
    def _add_eqangle(
        ...
        for x, y, xy in [(a, b, ab), (c, d, cd), (m, n, mn), (p, q, pq)]:
            ...
            if {x, y} == {x_, y_}:
                continue
            if deps:
                deps = deps.extend(self, 'eqangle', list(
                    args), 'para', [x, y, x_, y_])
    ...
    def add_eqangle(
        ...
        if (
            ...
            add += self._add_eqangle(a, b, c, d, m, n,
                                     p, q, ab, cd, mn, pq, deps)
    ...
    def add_aconst(
        ...
        for x, y, xy in [(a, b, ab), (c, d, cd)]:
            ...
            if {x, y} == {x_, y_}:
                continue
            if deps:
                deps = deps.extend(self, 'aconst', list(
                    args), 'para', [x, y, x_, y_])
            args[2 * i - 2] = x_
            args[2 * i - 1] = y_
    ...
    def _add_eqratio(
        ...
        for x, y, xy in [(a, b, ab), (c, d, cd), (m, n, mn), (p, q, pq)]:
            ...
            if deps:
                deps = deps.extend(self, 'eqratio', list(
                    args), 'cong', [x, y, x_, y_])
            args[2 * i - 2] = x_
            args[2 * i - 1] = y_
    ...
    def add_eqratio(
        ...
        if (
            ...
            add += self._add_eqratio(a, b, c, d, m, n,
                                     p, q, ab, cd, mn, pq, deps)
    ...
    def add_clause(
        ...
        def range_fn() -> (
            ...
            for c in clause.constructions:
                ...
                for n in cdef.numerics:
                    ...
                    args = list(
                        map(lambda x: self.get(x, lambda: int(x)), args))
        ...
        is_total_free = (
            len(
                clause.constructions) == 1 and clause.constructions[0].name in FREE
        )
        is_semi_free = (
        ...
        for p in new_points:
            # to generate txt logs.
            p.why = sum([d.why for d in new_points_dep], [])
            p.group = new_points
        ...
        for p in new_points:
            ...
            for x in p.rely_on:
                if not hasattr(x, 'base_rely_on'):
                    x.base_rely_on = set()
            p.base_rely_on = set.union(
                *[x.base_rely_on for x in p.rely_on] + [set()])
    ...
    def all_eqangles_8points(self) -> Generator[tuple[Point, ...], None, None]:
        ...
        angss.extend([[ang]
                     for ang in self.type2nodes[Angle] if ang.val is None])
        ...
        noang_ds = [d for d in self.type2nodes[Direction]
    ...
    def all_eqratios_8points(self) -> Generator[tuple[Point, ...], None, None]:
        ...
        ratss.extend([[rat]
                     for rat in self.type2nodes[Ratio] if rat.val is None])
        ...
        norat_ls = [l for l in self.type2nodes[Length]
                    if not l.neighbors(Ratio)]
